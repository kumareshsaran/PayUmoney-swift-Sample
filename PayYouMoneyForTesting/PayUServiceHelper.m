...
-(void) getPayment:(UIViewController*)controller :(NSString*)email :(NSString*)phone :(NSString*)name :(NSString*)amount :(NSString*)trxnID :(NSString*)KEY didComplete:(PayUCompletionBlock)getPaymentBlock didFail:(PayUFailuerBlock)failBlock{
    self.completionBlock = getPaymentBlock;
    self.failuerBlock = failBlock;
    self.parentController = controller;
    [self setParam:email :phone :name :amount :trxnID :KEY];
}


- (void)setParam: (NSString*)email : (NSString*)phone : (NSString*)firstName :(NSString*)amount :(NSString*)trxnID :(NSString*)KEY {
    PUMTxnParam *txnParam= [[PUMTxnParam alloc] init];
    
    
#pragma mark Sandbox Crenditials
//    txnParam.environment = PUMEnvironmentTest;
//    txnParam.key = @"DtUDmExp";
//    txnParam.merchantid = @"6299824";
//    self.payu_salt = @"ecVV771WpO";
...
//    self.payu_salt = @"";
    txnParam.environment = PUMEnvironmentProduction;
    txnParam.key = KEY; //Your mergent ID
    txnParam.merchantid = @"Ls8opZ4M"; //Your mergent ID
    self.payu_salt = @"KCqrs3m1jb";  //Your mergent salt
    ...
    txnParam.surl = @"https://7elevencabs.com/payu/success/wallet";
    txnParam.furl = @"https://7elevencabs.com/payu/failure/wallet";
